<template>
  <div class="card-main">
    <div class="card-settings" :style="{ height, width }">
      <div class="card-container" v-for="slide in props.wordsArray" :key="slide.key"
        :style="{ transform: `translateX(-${currentIndex * 105}%)` }">
        <div class="card-inner">
          <div class="card-face-key" tabindex="0" :style="{ backgroundColor }" :class="{ active: !state }"
            @click="changeSide" aria-label="Card front">
            {{ slide.key }}
            <p class="helper">Нажмите на карточку, чтобы перевернуть ее</p>
          </div>
          <div class="card-face-value" tabindex="0" :style="{ backgroundColor }" :class="{ active: state }"
            @click="changeSide" aria-label="Card back">
            {{ slide.value }}
            <p class="helper">Нажмите на карточку, чтобы перевернуть ее</p>
          </div>
        </div>
